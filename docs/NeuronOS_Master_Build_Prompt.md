...
9. The mockups — these are the visual and interaction source of truth. If your implementation and the mockup disagree on layout, states, or copy, the mockup wins unless you flag the discrepancy and get it resolved. Two distinct sets exist, covering different screens — read both:
   - `neuronos_ai_actions_automations.html`, `neuronos_projects_meetings.html`, `neuronos_project_meeting_detail.html` — interactive HTML, covering AI Actions/Automations detail and the Projects/Meetings list + detail views.
   - `UI_design1.png`, `UI_design2.png` — the full-product static mockups covering Pulse (populated/post-onboarding state), Customers, Knowledge Hub, AI Actions (queue preview), Automations (list preview), Reports, and Settings, plus a mobile app preview and a dark-mode preview. **These are the actual mockups Blueprint §6.1 refers to as "confirmed via existing high-fidelity mockups" for Pulse/Customers/Knowledge/Reports/Settings** — they were added to the repo after the HTML mockups and after most of this document set was written, so no other doc names them explicitly yet. Cross-check new screen work against these, not just the HTML set.
