...
- **Geography (confirmed this pass — previously an implicit, unstated assumption):** **India-first.** Every monetary example in this document (₹8L, ₹4,80,000) and the Database Spec's `currency CHAR(3) DEFAULT 'INR'` were already assuming this; it had just never been said plainly as a scoping decision. This is a confirmed product decision, not a guess the team is expected to re-derive from the examples: default currency is INR, and go-to-market focus is Indian founders/small teams before any other geography. `currency` remains a genuine per-organization field (not hardcoded) precisely because this is a default, not a hard constraint — a non-Indian design partner is not blocked, just not who the product is being designed around first.
...
Pulse (Home), Customers (list + detail), Knowledge (hub + document view), AI Actions, Automations, Reports, Settings — all confirmed via existing high-fidelity mockups: `docs/mockups/UI_design1.png` (Pulse populated/post-onboarding state, Customer Intelligence, AI Workspace, Knowledge Hub preview, AI Actions/Automations/Integrations preview, mobile app preview, dark-mode preview) and `UI_design2.png` (Customers list, Knowledge Hub, AI Actions queue, Automations list, Reports, Settings). **Note the two Pulse states are not in tension:** `UI_design1.png` shows Pulse once an org has real data (Business Health populated, customers/tasks/insights present) — the cold-start empty state (`onboarding_completed_at IS NULL`) specified in §4.4 and `NeuronOS_Onboarding_Spec.md` is a *different, earlier* state of the same screen, not a competing design.
...
- v1.2 → v1.3 (Phase 0 kickoff pass): confirmed India-first/INR as an explicit scoping decision in §1.7 rather than an implicit one inferred from examples. Resolved several gaps surfaced by a fresh read of the full document set before Phase 0 build began: `users.email` is now globally unique rather than per-organization (Database Spec §1.2 — per-org uniqueness was only in service of multi-org-per-person, which isn't supported at MVP, and it broke `POST /auth/login`'s ability to disambiguate an email against a specific organization); added the missing `POST /auth/signup` and `POST /auth/invitations/{token}/accept` endpoints (API Spec §1) without which Phase 0's own exit criteria — create an org, invite a teammate — could not actually be completed end-to-end; added `POST /reports/import` (API Spec §9) to back the Roadmap's existing "manual/CSV-fed" Reports promise, which had no endpoint; made the RLS `SET LOCAL` pattern's dependency on session-mode (not transaction-mode) connection pooling an explicit, documented infrastructure decision (Database Spec §0.1) rather than whatever a hosting provider defaults to; and settled that the full database schema (MVP + Phase 2 tables) is created in Phase 0 via Alembic, while application/API code remains scoped strictly to each Roadmap phase (Database Spec §9).
